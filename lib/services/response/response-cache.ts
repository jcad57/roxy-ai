...
import type { ResponseSuggestion, ResponseContext, CachedResponse } from '@/lib/types/response';

const RESPONSE_CACHE_KEY = 'mailmind-response-cache-v1';
...
export const ResponseCache = {
   ...
   * Cache a response
   */
  set(emailId: string, response: ResponseSuggestion, context: ResponseContext): void {
    try {
      const cache: CacheStore = JSON.parse(
        localStorage.getItem(RESPONSE_CACHE_KEY) || '{}'
      );
