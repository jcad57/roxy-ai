...
interface MailMindDB extends DBSchema {
  emails: {
    key: string;
    value: RawEmail;
...
const DB_NAME = 'mailmind-db';
const DB_VERSION = 1;

let dbInstance: IDBPDatabase<MailMindDB> | null = null;

/**
 * Initialize and open the IndexedDB database
 */
async function getDB(): Promise<IDBPDatabase<MailMindDB>> {
  if (dbInstance) return dbInstance;

  dbInstance = await openDB<MailMindDB>(DB_NAME, DB_VERSION, {
    upgrade(db) {
      // Create emails store
      ...
      if (!db.objectStoreNames.contains('enrichment')) {
        const enrichmentStore = db.createObjectStore('enrichment', { keyPath: 'emailId' });
...
export const EnrichmentDB = {
  ...
  async needsAnalysis(emailId: string, maxAge: number = 7 * 24 * 60 * 60 * 1000): Promise<boolean> {
